...
//查询数据
function show_outStock(pageNum) {
    $.ajax({
        url: getProjectPath() + "/outorderandwarehouse?pageNum=" + pageNum,
        type: "POST",
...
function build_page_nav(result) {
    ...
    if (result.extend.orders.hasPreviousPage == false) {
        ...
        prePageLi.click(function () {
            show_outStock(result.extend.orders.pageNum - 1);
        });
    }
    ...
    if (result.extend.orders.hasNextPage == false) {
        ...
        lastPageLi.click(function () {
            show_outStock(result.extend.orders.pages);
        });
    }
...
$(document).on("click", ".del-btn", function () {
    ...
    if (confirm("确认删除编号为" + orderNumber + "的出库单吗？")) {
        //确认，发送ajax请求删除
        $.ajax({
            url: getProjectPath() + "/deleteoutorder",
            type: "POST",
            data: "orderId=" + orderId,
            success: function () {
                show_outStock(currentPage);
            }
        });
    }
});
...
function getOrder(orderId) {
    $.ajax({
        url: getProjectPath() + "/outorderandwarehouse",
        type: "POST",
        data: "orderId=" + orderId,
        success: function (result) {
...
$(document).on("click", "#order_update_btn", function () {
    ...
    if (validate_form("update")) {
        $.ajax({
            url: getProjectPath() + "/updateoutorder",
            type: "POST",
...
$("#order_save_btn").click(function () {
    ...
    if (validate_form("add")) {
        // 不存在发送ajax请求修改数据
        $.ajax({
            url: getProjectPath() + "/addoutorder",
            type: "POST",
            ...
            success: function () {
                ...
                $.ajax({
                    ...
                    success: function (result) {
                        //显示当前添加行
                        show_outStock(result.extend.orders.total);
                    }
                });
            }
        });
...
$("#del_all_btn").click(function(){
    ...
    if(confirm("确认删除【"+orderNumbers+"】的出库单吗？")){
        //发送ajax请求删除
        $.ajax({
...
$(document).on("click", ".find-btn", function () {
    ...
    window.location.href = getProjectPath() + "/showoutitem?" + data;
});
//判断当前有单据中没有明细
