...
$(function () {
    // 获取所有货品
    $.ajax({
        url: getProjectPath() + "/cargo",
        type: "GET",
        success: function (result) {
            build_select(result.extend.cargos);
        }
    });
    show_item(1);
});
...
$(document).on("click", ".del-btn", function () {
    ...
    if (confirm("确认删除编号为" + orderItemNumber + "的入库单明细吗？")) {
        //确认，发送ajax请求删除
        $.ajax({
            url: getProjectPath() + "/deleteinorderitem",
            type: "POST",
...
$("#item_save_btn").click(function () {
    ...
    if (validate_account("#add_account")) {
        $.ajax({
            url: getProjectPath() + "/addinorderitem",
            type: "POST",
...
$(document).on("click", "#account_update_btn", function () {
    ...
    if (validate_account("#update_account")) {
        $.ajax({
            url: getProjectPath() + "/updateinorderitem",
            type: "POST",
